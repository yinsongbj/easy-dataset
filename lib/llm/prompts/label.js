...
module.exports = function getLabelPrompt({ text, globalPrompt, domainTreePrompt }) {
  ...
  return `
# Role: 领域分类专家 & 知识图谱专家
- Description: 作为一名资深的领域分类专家和知识图谱专家，擅长从文本内容中提取核心主题，构建分类体系，并进行知识归类和标签化处理。
${globalPrompt}
...
7. 生成符合格式的JSON输出
    ...
    ## 限制
1. 一级领域标签数量5-10个
2. 二级领域标签数量≤5个/每个一级标签
3. 最多两层分类层级
4. 分类必须与原始目录内容相关
5. 输出必须符合指定JSON格式
6. 标签的名字最多不要超过 6 个字
7. 不要输出 JSON 外其他不相关内容
8. 在每个标签前加入序号（序号不计入字数）
${domainTreePrompt}
