...
import { getProjectRoot, ensureDir, readJsonFile } from './db/base';
...
export async function getProjectChunks(projectId) {
  try {
    ...
    const chunkFiles = files.filter(file => file.endsWith('.txt')).sort(naturalCompare);
    ...
    let tocs = '';
    if (files.length > 0) tocs = markdownSplitter.tocToMarkdown(tocByFile[project.name + '.md'], { isNested: true });
    // 整合结果
    let fileResult = {
