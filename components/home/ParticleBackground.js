    ...
    const drawParticles = () => {
      ...
      particles.forEach(particle => {
        ...
        if (distance < hoverRadius) {
          ...
          // 粒子远离鼠标，并受鼠标速度影响
          particle.speedX = -Math.cos(angle) * force * mouseFactor + 
                           particle.originalSpeedX + 
                           (mouseSpeed.x * 0.05);
          particle.speedY = -Math.sin(angle) * force * mouseFactor + 
                           particle.originalSpeedY + 
                           (mouseSpeed.y * 0.05);
        } else {
          // 逐渐恢复原始速度
    ...
    const drawLines = () => {
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distance < maxDistance) { // 只在粒子距离小于maxDistance时绘制连线
            ctx.beginPath();
